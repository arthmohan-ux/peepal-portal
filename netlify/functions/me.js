...
exports.handler = async (event) => {
  ...
  if (IS_DEV) {
    return { statusCode: 200, headers, body: JSON.stringify({ email: 'arth.mohan@peepalconsulting.com', name: 'Arth Mohan' }) };
  }
  ...
  try {
    ...
    return { statusCode: 200, headers, body: JSON.stringify({ email: payload.email, name: payload.name }) };
  } catch {
